...
impl Room {
    ...
    pub async fn listen(
        game_id: u32,
        admin: Connection,
        mut receiver: UnboundedReceiver<GameEvents>,
    ...
    async fn broadcast(
        room_id: u32,
        admin: Connection,
        mut rx: UnboundedReceiver<GameEvents>,
        to_lobby: UnboundedSender<LobbyEvents>,
    ) {
        ...
        while let Some(event) = rx.recv().await {
            match event {
                ...
                GameEvents::Quit(user_id, forward) => {
                    ...
                    to_lobby
                        .send(LobbyEvents::PlayerCount(room_id, user_id, len))
                        .unwrap();
                    if len == 0 {
